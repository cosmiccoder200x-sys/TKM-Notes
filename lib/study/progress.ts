...
export function getSubjectProgress(subjectCode: string): Record<string, ModuleProgress> {
  return safeRead()[subjectCode] ?? {};
}

export function getModuleProgress(subjectCode: string, moduleId: string): ModuleProgress | undefined {
  return getSubjectProgress(subjectCode)[moduleId];
}

export function recordAttempt(subjectCode: string, moduleId: string, result: AttemptResult) {
  const map = safeRead();
  ...
  map[subjectCode] = subject;
  safeWrite(map);
}

export function markModuleReviewed(subjectCode: string, moduleId: string) {
  const map = safeRead();
  const subject = map[subjectCode] ?? {};
  const rec = subject[moduleId] ?? { attempts: 0, correct: 0, partial: 0, incorrect: 0, reviewed: false };
  rec.reviewed = true;
...
export function resetSubject(subjectCode: string) {
  const map = safeRead();
